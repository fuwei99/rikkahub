...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("  - `workspace_write_file` / `workspace_edit_file`: create files, or make precise edits to existing files. These create restorable backups before writing.")
    appendLine("  - `workspace_apply_patch`: apply Git-style unified diff patches for multi-file text changes, create/delete/rename included. It creates a backup before non-dry-run writes.")
    appendLine("  - `workspace_list_backups` / `workspace_restore_backup`: inspect or restore backups created by file-changing tools.")
    appendLine("  - `workspace_shell`: run shell commands (the files area is mounted at /workspace).")
    appendLine("- Prefer `workspace_edit_file` for single targeted edits, `workspace_apply_patch` for multi-file diffs, and `workspace_shell` for commands/tests/move/delete operations outside text patches.")
    ...
    val externalMounts = workspace.externalMountConfigs()
    if (externalMounts.isNotEmpty()) {
        appendLine("- External mounts available in shell and workspace tools:")
        externalMounts.forEach { mount ->
            appendLine("  - `${mount.normalizedTargetPath()}` from `${mount.sourcePath}` (${if (mount.writable) "read/write" else "read-only"}).")
        }
        appendLine("  If an external Android public directory appears to contain only folders or no files, ask the user to grant All files access in Workspace > External mounts.")
    }
